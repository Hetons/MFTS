...
def get_args():
    ...
    parser.add_argument(
        ...
        metavar="INPUT",
        type=str,
        default="decision-entropy-ntree_30-ratio_0.8-pack_4-fold_3",
    )
...
def data_load(valid_data, valid_labels, test_data, test_labels, seq_leng):
    ...
    valid_dataloader.append(
        DataLoader(
            Mydataset(
                np.array(valid_data, dtype="int32"),
                np.array(valid_labels, dtype="int32"),
                seq_leng,
            ),
            batch_size=64,
            shuffle=True,
    ...
    test_dataloader.append(
        DataLoader(
            Mydataset(
                np.array(test_data, dtype="int32"),
                np.array(test_labels, dtype="int32"),
                seq_leng,
            ),
            batch_size=64,
            shuffle=True,
...
def valid(model, device, valid_loader, i, loss_fn):
    ...
    print(
        "Testing fold {} current total loss:{:.4f} current total acc:{:.4f}".format(
...
def test(model, device, test_loader, i, loss_fn):
    ...
    with torch.no_grad():
        ...
        acc1 = balanced_accuracy_score(y_true_trans1, y_predict_trans1)
        test_acc1 = 100.0 * acc1
    print("Valid fold {} loss:{:.4f} acc:{:.4f}".format(i, avg_loss, test_acc))
    print(
        "Valid fold {} current total loss:{:.4f} current total acc:{:.4f}".format(
            i, avg_loss, test_acc1
        )
    )
    print("---------------------------------------------------")
