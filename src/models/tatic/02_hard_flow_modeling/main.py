...
def get_args():
    ...
    parser.add_argument(
        ...
        metavar="INPUT",
        type=str,
        default="./in_channel_{}-in_kernel_{}-layers_{}-seq_len_{:0>2d}-ker_size_{:0>2d}-fold_{}",
    )
...
outname = args.model_name.format(
    input_channel, args.in_ker_num, args.layers, args.seq_len, kernel_size, fold
)
...
def data_load(new_data, seq_leng):
    ...
    train_dataloader.append(
        DataLoader(
            Mydataset(
                np.array(new_data[0], dtype="int32"),
                np.array(new_data[1], dtype="int32"),
                seq_leng,
            ),
    ...
    valid_dataloader.append(
        DataLoader(
            Mydataset(
                np.array(new_data[2], dtype="int32"),
                np.array(new_data[3], dtype="int32"),
                seq_leng,
            ),
            batch_size=batch_size,
            shuffle=True,
    ...
    test_dataloader.append(
        DataLoader(
            Mydataset(
                np.array(new_data[4], dtype="int32"),
                np.array(new_data[5], dtype="int32"),
                seq_leng,
            ),
            batch_size=batch_size,
            shuffle=True,
...
def train(model, device, train_loader, optimizer, epochs, i, loss_fn):
    ...
    for idx, (data, target) in enumerate(train_loader):
        ...
        if idx % 100 == 99:
            print(
                "Fold {} Train Epoch: {}, iteration: {}, Loss: {}".format(
                    i, epochs, idx + 1, loss.item()
                )
            )
    ...
    print("---------------------------------------------------")
    print(
        "Fold {} epoch:{}  training Loss:{:.4f} training acc:{:.4f}".format(
            i, epoch, avg_loss, train_acc
        )
    )
...
def test(device, test_loader, i, loss_fn, save_dir, outname):
    ...
    model = TCN(
        ...
        dropout=dropout,
        vocab_text_size=vocab_text_size,
        seq_leng=seq_leng,
    ).to(device)
